  ...
  return (
    <div className="space-y-6">
      <div className="header-gradient rounded-2xl p-6 animate-fade-in relative overflow-hidden">
        ...
        <Sparkles className="absolute right-6 top-1/2 -translate-y-1/2 w-16 h-16 text-primary-foreground/20" />
      </div>

      {/* Stats & Actions */}
      <div className="flex items-center justify-between animate-fade-in" style={{ animationDelay: "0.1s" }}>
        <div className="card-metric">
          <p className="text-xs text-muted-foreground mb-1">Total Rekomendasi</p>
          <p className="text-2xl font-bold text-foreground">{recommendations.length}</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 bg-success text-success-foreground rounded-lg text-sm font-medium hover:bg-success/90 transition-colors">
      ...
      <div className="space-y-4">
        {recommendations.map((rec, index) => (
          <div
            key={rec.id}
            className={`card-metric ${getCardBorder(rec.urgency)} animate-fade-in`}
          ...
          >
            ...
            <div className="bg-secondary/50 rounded-lg p-4 mb-4">
              <p className="text-xs font-semibold text-foreground mb-2">Rekomendasi Tindakan:</p>
              <p className="text-sm text-muted-foreground">{rec.actions}</p>
            </div>

            <div className="flex items-center gap-4 text-sm text-muted-foreground mb-4">
              <span>
                Area: <span className="font-semibold text-foreground">{rec.area}</span>
              </span>
              <span>
                Dampak:{" "}
