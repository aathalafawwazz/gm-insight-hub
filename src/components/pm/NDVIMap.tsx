...
export const NDVIMap = () => {
  ...
  return (
    <div className="card-metric animate-fade-in" style={{ animationDelay: "0.2s" }}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-foreground">Peta NDVI</h3>
      ...
      <div className="relative bg-gradient-to-br from-emerald-900/20 via-teal-800/30 to-green-900/20 rounded-xl overflow-hidden">
          ...
          {plots.map((plot) => (
            <g key={plot.id}>
              <rect
                ...
                width={plot.width}
                height={plot.height}
                className={`${getStatusColor(plot.status)} transition-all duration-200 cursor-pointer ${
                  hoveredPlot?.id === plot.id ? "opacity-100 stroke-white stroke-[0.5]" : "opacity-80"
                }`}
                rx="0.5"
        ...
        {hoveredPlot && (
          <div
            className="absolute z-10 bg-card/95 backdrop-blur-sm border border-border rounded-lg p-3 shadow-lg pointer-events-none"
            style={{
          ...
          >
            ...
            <div className="space-y-1 text-xs">
              <div className="flex justify-between gap-4">
                ...
                <span className="font-medium">{hoveredPlot.ndvi.toFixed(3)}</span>
              </div>
              <div className="flex justify-between gap-4">
                <span className="text-muted-foreground">Status:</span>
                <span className={`font-medium ${
                  hoveredPlot.status === "kritis" ? "text-danger" :
                  hoveredPlot.status === "buruk" ? "text-orange-500" :
                  hoveredPlot.status === "sedang" ? "text-warning" :
                  hoveredPlot.status === "baik" ? "text-lime-500" :
                  "text-success"
                }`}>
